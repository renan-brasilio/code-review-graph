...
- **Salesforce / Apex enhancements**: static call resolution
  ...
  queries, Salesforce lifecycle entry-point patterns in flow detection,
  `*.field-meta.xml` metadata indexing with resolved cross-field/relationship
  `REFERENCES` edges and `Field --BELONGS_TO--> Object` edges,
  `*.flow-meta.xml` indexing (**`SalesforceFlow`** nodes with `INVOKES` edges
  to Apex actions and subflows, `REFERENCES` edges to objects touched),
